"""
Evaluate MSE, LPIPS, SSIM for DragBench-DR editing results.

Source images: data/DragBench/openvid_format_dbscan_dr/{category}/{sample_id}/original_image.png
Edited images: outputs/dragbench/DragBench-DR/{method}/... (method-specific structure)

Output: one txt file per method under evaluation/DragBench-DR_similarity_results/
"""
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "outputs/evaluation/DragBench/DragBench_DR/similarity")
...
def collect_source_pairs(source_root: str) -> List[Tuple[str, str, str]]:
    """
    Walk dragbench-dr and collect (source_path, category, sample_id) for each leaf dir
    that contains original_image.png.
    Returns list of (source_path, category, sample_id).
    """
    pairs = []
    for cat in sorted(os.listdir(source_root)):
        ...
        for sample_id in sorted(os.listdir(cat_path)):
            ...
            if os.path.isfile(orig_path):
                pairs.append((orig_path, cat, sample_id))
    return pairs
...
def resolve_edited_path(
    ...
    """
    # Method-specific path resolution
    configs = {
        ...
        "ICRDrag": lambda: _resolve_icrdrag(method_root, category, sample_id),
    }
...
def _resolve_icrdrag(method_root: str, category: str, sample_id: str) -> Optional[str]:
    """ICRDrag: edit_image and source_image subdirs, same filename {category}_{sample_id}.jpg/.png."""
    edit_dir = os.path.join(method_root, "edit_image")
    if not os.path.isdir(edit_dir):
        return None
    for ext in (".jpg", ".png"):
        path = os.path.join(edit_dir, f"{category}_{sample_id}{ext}")
...
def collect_icrdrag_flat_pairs(
    icrdrag_dir: str,
    source_pairs: List[Tuple[str, str, str]],
) -> List[Tuple[str, str]]:
    pairs = []
    for src_path, category, sample_id in source_pairs:
        stems = (f"{category}_{sample_id}", sample_id)
        for ext in (".jpg", ".png"):
            for stem in stems:
                edited_path = os.path.join(icrdrag_dir, f"{stem}{ext}")
...
def _resolve_fastdrag(method_root: str, category: str, sample_id: str) -> Optional[str]:
    """FastDrag has a config-named subdir; use first subdir that contains category folders."""
    try:
        subdirs = [d for d in os.listdir(method_root) if os.path.isdir(os.path.join(method_root, d))]
        for sub in sorted(subdirs):
            path = os.path.join(method_root, sub, category, sample_id, "dragged_image.png")
            if os.path.isfile(path):
                return path
    except OSError:
        pass
...
def run_method(
    ...
    if method == "ICRDrag" and icrdrag_dir:
        ...
        pairs = []
        skipped = []
        for src_path, cat, sample_id in source_pairs:
            edt_path = resolve_edited_path(method, method_root, cat, sample_id)
            if edt_path:
                pairs.append((src_path, edt_path))
            else:
                skipped.append(f"{cat}/{sample_id}")
        if skipped:
    ...
    eval_src_root = method_root if method == "ICRDrag" else source_root
    eval_res_root = method_root if method == "ICRDrag" else result_root
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(f"# DragBench-DR Evaluation: {method} ({len(pairs)} pairs)\n")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Evaluate MSE, LPIPS, SSIM for DragBench-DR methods.")
    parser.add_argument("--source_root", default=SOURCE_ROOT, help="Root of dragbench-dr dataset.")
    parser.add_argument("--result_root", default=RESULT_ROOT, help="Root of DragBench-DR results.")
    ...
    print(f"Found {len(source_pairs)} source samples.")
