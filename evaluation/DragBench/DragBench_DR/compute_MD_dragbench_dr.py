"""
Compute MD (Motion/Drag Distance) for DragBench-DR dataset and 7 methods.

Dataset: data/DragBench/openvid_format_dbscan_dr — each leaf folder has original_image.png and meta_data.pkl (point pairs).
Results: outputs/dragbench/DragBench-DR/{method}/... (method-specific paths).
...
# Project root: script is evaluation/DragBench/DragBench_DR/compute_MD_dragbench_dr.py.
...
# Results under this script's directory: DragBench/DragBench_DR/MD_regiondrag
...
def collect_samples(source_root: str) -> List[Tuple[str, str, str]]:
    ...
    Returns list of (orig_path, category, sample_id).
    """
    samples = []
...
def resolve_edited_path(
    ...
    """
    Resolve edited image path for each method. Same logic as run_eval_dragbench_dr.py.
    """
...
def run_md_for_method(
    ...
    for orig_path, category, sample_id in tqdm(samples, desc=f"MD {method}"):
        ...
        results_per_sample.append((sample_name, md))
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute MD on DragBench-DR for all 7 methods."
    )
    parser.add_argument(
