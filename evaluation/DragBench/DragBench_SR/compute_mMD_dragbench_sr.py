"""
Compute m-MD (masked Mean Distance) for DragBench-SR dataset using DIFT.

Dataset: data/DragBench/openvid_format_dbscan_sr — flat structure, each {sample_id}/ has original_image.png
  and meta_data.pkl (points: [handle, target, ...], mask: HxW binary).
Results: outputs/dragbench/DragBench-SR/{method}/...
...
# Project root: script is evaluation/DragBench/DragBench_SR/compute_mMD_dragbench_sr.py.
...
def run_mmd_for_method(
    ...
    for orig_path, sample_id in tqdm(samples, desc=f"m-MD {method}"):
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute m-MD on DragBench-SR using DIFT and mask from meta_data.pkl."
    )
    parser.add_argument(
