...
class DragEvaluator:
    def __init__(self, sd_model_path=None):
        self.clip_loaded = False
        self.dino_loaded = False
        self.sd_loaded = False
        self.sd_model_path = sd_model_path or os.environ.get("EVAL_SD_MODEL_ID")
    ...
    def load_sd(self):
        if not self.sd_loaded:
            if not self.sd_model_path:
                raise ValueError("SD feature extraction requires --sd_model_path or EVAL_SD_MODEL_ID.")
            sd_path = self.sd_model_path
